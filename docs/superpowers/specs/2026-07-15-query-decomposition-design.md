...
| 2026-07-16 | Measured (second pass, n=72 dedicated sample): compound full coverage 3/9 → 5/9 (mean 0.64 → 0.75) with --decompose (run 1); 3/9 → 7/9 (mean 0.64 → 0.86) (run 2 — the two runs' full-coverage counts differ, expected nondeterminism from the LLM split, both recorded); red-card question 1/4 → 1/4 (unchanged both runs — still misses Law 3›1, Law 7›5, Law 10›2). Soft-deadline (`DECOMPOSE_SOFT_DEADLINE_MS=2500ms`, `DECOMPOSE_TIMEOUT_MS=4000ms`) miss rate on n=72 dedicated sample: 2/72 (2.8%) — comfortably under the 20% acceptance bar (spec §10.4); no graduated-escalation step needed. Simple-question added latency: p50 ~781ms, max ~2182ms (n=10) — under the 1500ms companion bar. Both bars passed cleanly on the first measurement; no tension between them, no escalation to Markus required. |
