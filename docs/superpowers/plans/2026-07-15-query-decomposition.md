...
- `npm run eval -- --decompose` spends real money (~a cent) and needs `.env.local`'s `ANTHROPIC_API_KEY` — it is already present; do not echo or commit it. Task 5 runs it twice to check for split nondeterminism in the compound-coverage numbers. The soft-deadline miss-rate measurement is a separate dedicated n=72 sample (spec §10.4), not derived from these two runs.
