...
**If the compound completeness check shows a regression on ANY of the 16
compound questions** (not just the 2 new Task 3 entries — a required
breadcrumb that a `searchChunks` check confirmed lands in the real top-8
retrieval is still not being cited in the generated answer): stop and
investigate before proceeding, same as the golden/paraphrase gate above —
per spec §5, an unresolved compound regression is not shippable, this is
a real stop-gate covering the whole compound set, not just something to
"check closely" on the 2 newest entries. Do not let this verification
step's own finding become a footnote carried forward instead of
resolved.
