...
> Last updated: 2026-07-20 (Part 1 — ingestion + retrieval — completed 2026-07-09. Part 2a — ask API, guardrails, gate calibration — completed 2026-07-12. Part 2b — UI, guardrail hardening, Railway deploy — completed 2026-07-13; PR #27 approved by a fresh-context review and merged 2026-07-14. Post-merge: PRs #32–#36 (issues #30/#31 fix, compound-eval tier, workflow-audit fixes, docs) all reviewed and merged 2026-07-15. Query decomposition — the compound-question fix — spec'd 2026-07-15 (PR #46), executed through two mid-stream redesigns, merged to `main` and deployed live 2026-07-19 (PR #50, Concept 19). Live testing after deploy surfaced two new, distinct RAG failure modes (issues #64, #65 — Concept 22) and a statistical-power lesson in the feature's own acceptance testing (Concept 20). Same day, second session: compound-eval harvest finished (PR #68, 2 mis-cited questions caught and fixed by review before merge); issue #64's fix designed end-to-end — a live-corpus investigation (Concept 24) informed a citation-integrity-preserving design (Concept 25), both the spec (PR #69) and implementation plan (PR #70) went through two rounds of Fable review each, catching real BLOCKERs every round — and the resumed-vs-fresh-review comparison run deliberately across both PRs became real evidence for an open process question (Concept 26). Design fully approved and merged; implementation itself deferred to a future session. **2026-07-20: issue #64 implemented and shipped** (Concept 27) — live production re-ingest gated on explicit human sign-off, merged as PR #72. **Same day, issue #65 (the generation-grounding bug) designed and shipped through a new author/reviewer split** — a cheap-tier session authored the spec and plan directly instead of the design being authored by the same strong reviewer model, specifically to test whether a different-author/cold-reviewer split is a stronger independence guarantee than a single model reviewing its own prior work (Concept 28). The resulting PR went through 5 independent review rounds, converging from 2 real BLOCKERs down to zero — both BLOCKERs turned out to share one root-cause shape, written up as a portable retrospective finding rather than patched ad hoc (Concept 29). Merged as PR #73. Traces to `docs/superpowers/specs/2026-07-08-laws-rag-design.md`.)
...
**Execution and measurement:** The measured baseline is what justified decomposition over the cheaper alternative of just raising k — 2 of the 9 compound questions, including the original live failure, never reached full coverage even at k=24, so no amount of retrieval-count tuning alone would have closed the gap. The soft-deadline race (spec §3/§4/§7, added mid-execution after a task review) is the piece that makes the latency story honest: a naive `Promise.all([searchChunks, decompose])` — the reference shape the implementation plan originally specified — would have blocked *every* request, simple or compound, on decompose's full latency; the elapsed-aware race instead bounds how long `/api/ask` waits on decompose before falling back to the baseline path, so simple questions pay a small, capped tax instead of the full call. The fallback contract (decompose failures — malformed output, refusal, API error, timeout — all resolve to the exact single-pass path) means the decomposer can structurally never make retrieval worse than the pre-decomposition system, only better or unchanged. Real measurement (Task 5, n=72 dedicated sample against a live Anthropic call) found the soft deadline misses only 2.8% of the time at `DECOMPOSE_SOFT_DEADLINE_MS=2500ms`, and simple-question added latency stayed at p50 ~781ms (max ~2182ms, n=10) — both comfortably inside their acceptance bars. Compound full coverage rose from 3/9 at baseline to 5/9–7/9 with decomposition (two `--decompose` eval runs differed due to the LLM split's inherent nondeterminism, both recorded rather than picking the more favorable one). **Merged and deployed 2026-07-19** (PR #50 → `main` → Railway) after a fresh final whole-branch review found no Critical/Important issues across all 20 commits; the live human end-to-end check (the same red-card question that started the whole investigation) confirmed the app behaves as expected, with the known incomplete-coverage limitation reproducing exactly as measured — see Concept 22 for what testing it live surfaced next.

---

### 20. Statistical power in a hand-rolled eval bar

**Simple version:** A "must pass 80% of the time" rule sounds precise, but if the test only ever runs 9 times, one unlucky miss already looks like an 11% failure rate — the bar wasn't actually measuring what it claimed to measure until the sample size grew.

**The longer version:** The soft-deadline mechanism's original acceptance criterion was "miss rate under 20%," checked against the existing 9-question compound eval set. At n=9, a single miss is already ~11.1%, and two misses cross the 20% line — meaning the bar could pass or fail almost by coin-flip depending on which single call happened to be slow that particular run, independent of whether the underlying constant was actually well-tuned. This was diagnosed deliberately, not discovered by accident: a second design consultation asked what sample size the bar actually needed to be trustworthy, and the fix replaced the underpowered n=9 check with a dedicated n=72 direct-`decompose()` sample (8 repeats × 9 questions) built specifically to measure the miss rate itself, decoupled from the unrelated compound-coverage measurement it had been piggybacking on.

**Interview talking point:** "My first acceptance bar for a latency feature was 'under 20% miss rate,' checked against 9 samples — and at n=9, one unlucky miss is already an 11% failure rate, so the test was really measuring noise, not the constant. I caught this by asking what sample size the bar needed to actually be trustworthy, then built a dedicated 72-sample measurement instead of reusing an unrelated eval tier's sample count for a different, more sensitive question. A pass/fail bar is only as good as the sample size behind it — 'we have a number' isn't the same as 'we have a measurement.'"

---

### 21. A process rule that only cost a retro because it was caught immediately

**Simple version:** A rule said "design revisions get their own review branch." When a bug fix mid-build turned out to need a real design change, the design-doc half correctly went through its own branch — but the paired implementation-plan half got treated as "just translating the design into code" and landed directly on the feature branch instead. Same rule, same situation, only half-applied.

**The longer version:** A task review during query decomposition's build found the implementation plan's own reference code was flawed (the naive `Promise.all` race described in Concept 19). The resulting spec amendment correctly got its own `docs/` branch and three rounds of independent review. The paired plan revision — translating that new design into concrete task steps — was committed directly onto the in-progress feature branch instead, on the reasoning that it was mechanical translation, not new design work. It wasn't: two real defects were later found in that plan revision (a test-hang from a WebCrypto/fake-timers interaction, and a required measurement step that had been silently dropped), proving it needed the same independent-review rigor the spec got. A retro named the actual gap precisely — "spec revision" and "plan revision" were being treated as different risk classes when they're not — and the fix was applied at the source (a global workflow rule, plus the subagent-driven-development skill's own BLOCKED-handling checklist) the same session, not left as a note to remember next time.

**Interview talking point:** "The interesting part of this incident isn't the mistake, it's that the rule already existed and was still only half-followed — the spec revision correctly got its own branch, the plan revision that translated it into code didn't, because 'just translating the design' felt like execution rather than design. A retro traced that exact reasoning gap and closed it by editing the actual skill and workflow files that generate the behavior, not by writing 'remember to do this' somewhere. Rules that look complete on paper often have an unstated boundary that only a real scenario reveals."

---

### 22. Testing my own system in production and finding two different kinds of failure

**Simple version:** After shipping the compound-question fix, I asked the live app the two hardest questions I could think of. Both answers were wrong — but wrong in two completely different ways, which mattered for what to do about each one.

**The longer version:** Live testing after deploy surfaced two distinct RAG failure modes on the same day, and telling them apart required going past the app's own answer text and querying the underlying vector database directly. The first (issue #64): a question about a team reduced below the minimum player count got an answer built around a penalty-shoot-out-specific exception, because the actually-governing general rule never made the retrieval system's top-8 candidates at all — a pure retrieval problem, invisible from the answer text alone, only provable by calling the retrieval function directly and inspecting the real candidate list. The second (issue #65): a handball-deflection question got a confident, specific, and incorrect ruling on one run, and a correct honest hedge on an identical retry — the retrieved passages were topically related but described a different scenario (a player's own handball voiding their own goal, not an opponent's deflection), and the model filled the gap with an unsupported inference rather than recognizing the mismatch. Same app, same day, two failures that need entirely different fixes: retrieval tuning for one, tighter grounding instructions for the other. Neither was guessed at — both were confirmed by reading the actual retrieved chunk text from the database, not by re-reading the model's own explanation of itself.

**Interview talking point:** "I don't trust a language model's self-report when something looks wrong — I go to the source of truth instead. Both of these bugs looked similar from the outside — a wrong football ruling — but querying the actual retrieval results showed they were different failures at different layers: one was a chunk that never got retrieved, the other was a chunk that got retrieved but didn't actually answer the question asked, and the model guessed anyway. Diagnosing which layer is actually broken before proposing a fix is the difference between a plausible-sounding patch and an actual root cause."

---

### 23. Verification over trust, twice in one session

**Simple version:** Two different times in one session, checking a claim against the actual data source instead of accepting a plausible-sounding answer caught something a confident guess would have missed — once curating new test questions, once correcting my own mistake.

**The longer version:** While curating five new compound-eval questions from real-world referee controversies, the first draft assigned each question a law-section citation from general football knowledge. One of those five — a question about a second yellow card for simulation — was checked directly against the live corpus before being finalized, and the check showed both facts the question needed actually lived inside the *same* database section, not two separate ones as assumed; it wasn't a genuine two-section test after all, and was swapped for a verified replacement before being shown to the project owner for sign-off. This mirrors the same discipline used to diagnose issues #64 and #65 (Concept 22): query the database directly rather than trust a description of what should be there. A related, non-technical instance of the same principle happened the same session: a summary mischaracterized a parked task's ownership ("needs a Fable-tier session") when the source document already said the opposite explicitly. Caught by the project owner, the error traced back to a stale memory note rather than the authoritative doc, and the fix corrected the actual source rather than just the one wrong sentence already spoken.

**Interview talking point:** "The theme across several things I did this session is the same: don't trust a plausible-sounding claim, check it against whatever the actual source of truth is — a database, a source document, a memory file — before acting on it or presenting it as decided. It caught a bad eval question before it shipped, it's how I told apart two different RAG bugs, and when I did get something wrong — mischaracterizing where a task should be handled — the fix was tracing the error back to its actual source rather than just patching the one sentence someone caught."

---

### 24. Measuring before deciding a fix's shape: reproduction and a systemic check, not intuition

**Simple version:** Before designing a fix for a retrieval bug, I didn't just fix the one broken question — I first checked, with real data, whether the same kind of problem existed anywhere else, and I made sure the original bug actually reproduced reliably before touching anything.

**The longer version:** For issue #64 (a real question never surfacing its governing rule), the investigation ran in two phases before any fix was designed. First, reproduction: the exact question was run against the live retrieval pipeline 5 times in a row, producing identical results each time (same missing chunk, same similarity scores to the third decimal) — confirming a deterministic, real bug, not embedding-API noise. Second, a systemic check: 4 questions targeting other numeric-threshold rules across different Laws, deliberately phrased to avoid the target rule's own vocabulary (the same shape of gap as the original bug), plus 2 more questions from a different rule area entirely — all 6 retrieved correctly. That result ruled out a corpus-wide embedding-quality problem and justified a narrow, single-chunk fix instead of a general-purpose mechanism (a synonym glossary or a query-rewriting layer) that would have been solving a problem with no evidence behind it.

**Interview talking point:** "Before I design a fix, I want to know two things: does this actually reproduce, and is it one bug or a symptom of something bigger. For a retrieval-gap bug, I ran the exact failing question five times to rule out flakiness, then ran six deliberately similar questions across other parts of the corpus to check whether the gap was systemic. All six worked fine — that told me the fix should be narrow and targeted, not a general mechanism built on a hunch. Measuring the blast radius before choosing a fix shape is what kept a one-line fix from turning into unnecessary new infrastructure."

---

### 25. Citation integrity as a design constraint: separating what's searched from what's shown

**Simple version:** When I needed to help one specific rulebook paragraph get found more easily by the search, I couldn't just add helpful extra words to that paragraph — because then the app would be showing users text that isn't actually what the official rulebook says. So the fix keeps two separate copies: one for searching, one for showing.

**The longer version:** The chosen fix for issue #64 adds a new database column, `embedding_text`, that's nullable and empty for every chunk except one. For that one chunk (the rule about a match needing at least seven players), `embedding_text` holds the real rule text plus one added clarifying sentence, and only that enriched text feeds the embedding step. The existing `content` column — what the app actually displays as a cited passage — is completely untouched, still exactly the verbatim official text. This design was chosen specifically because the alternative (editing the displayed text directly) would mean showing users non-official commentary as if it were an exact quote from the IFAB Laws of the Game, which conflicts with the project's core premise: every answer is supposed to be strictly traceable to the real rulebook. The separation costs one schema column and one small ingest-time change, and preserves a guarantee that matters more than the convenience of a single shared field.

**Interview talking point:** "This project's whole value proposition is that every answer traces back to the real rulebook text — so when I found a retrieval bug that a bit of extra wording would fix, I couldn't just edit the passage the app shows people, because that would mean the citation isn't actually verbatim anymore. I split it into two fields instead: one that only feeds the search index, and one that's still exactly the official text shown to the user. It's a small schema change, but it means the fix improves findability without ever compromising what the app promises about its citations."

---

### 26. Multi-round AI-assisted review: real evidence that a fresh reviewer catches what a resumed one doesn't

**Simple version:** When I use an AI reviewer to check a PR, then fix what it finds and ask the same reviewer to confirm the fix, is that different from asking a brand-new reviewer who's never seen the PR before? I actually tested this twice in one session, and both times the answer was yes.

**The longer version:** This project uses a stronger AI model as an independent code reviewer on every PR. There's an open question about whether a review-and-confirm cycle needs a genuinely fresh reviewer dispatch each round, or whether resuming the same reviewer's thread to check "did you fix what I flagged" is good enough. This session ran both, deliberately, on two consecutive PRs (a design spec and its implementation plan): a resumed thread confirmed each fix and did real additional work (catching new issues, not just rubber-stamping its own prior finding) — but a genuinely fresh reviewer, dispatched afterward with no memory of the prior rounds and explicit instructions not to defer to earlier comments, still caught a materially different, non-overlapping set of real issues both times. On the implementation plan specifically, the fresh reviewer caught a missing regression-test check on exactly the one area most plausible to be affected by the change being made — the kind of contextual judgment that's easier to notice reading something cold than re-reading something you've already anchored on.

**Interview talking point:** "I don't assume a review process works just because it feels rigorous — I test it. There's a real question about whether an AI reviewer confirming its own fix is as good as a completely fresh one, so I ran both on the same PRs and compared what each one actually caught. The resumed reviewer wasn't rubber-stamping — it did real work — but the fresh reviewer still found real, different issues both times, including one that mattered specifically because of what the change touched. That's the kind of evidence that should drive a review-process decision, not just intuition about which feels more thorough."

---

### 27. A live production migration gated on an explicit human confirmation, enforced by the platform itself

**Simple version:** Some fixes require actually rewriting live data, not just changing code. For this one — re-computing the search index for the entire rulebook — the AI agent wasn't allowed to just do it. It had to ask first, and the tooling itself refused to let that step slip through unnoticed.

**The longer version:** Issue #64's fix required a real production operation: re-embedding all 118 chunks in the live database so one corrected entry would take effect. Before that step, a cheaper "dry-run" validation ran first — measuring, with real numbers, whether the fix's target similarity score would clear a safety margin (0.0283 above a 0.02 threshold) rather than just barely squeaking by. Once that gate passed, the agent still didn't proceed automatically: the platform's own safety classifier blocked the dispatch attempt and required an explicit human confirmation before a real, data-mutating operation against production could run. The confirmation wasn't a rubber stamp on an already-decided action — it was requested specifically because the operation was irreversible-ish and touched live user-facing data, with the dry-run result already in hand as supporting evidence, not as a substitute for asking.

**Interview talking point:** "Not every fix should be allowed to run unattended, even in an otherwise automated agentic workflow. This one needed to re-embed the entire live search index, so I built in two layers: a cheap dry-run that measures the fix's safety margin with real numbers before touching anything, and then an explicit human confirmation gate for the actual production write — enforced by the tooling itself, not just a comment saying 'ask first.' The dry-run doesn't replace the human judgment call; it gives the human better information to make it with."

---

### 28. Piloting a different author/reviewer split for AI-assisted design work

**Simple version:** Usually, the same strong AI model both writes a design document and then reviews its own work (with a clean memory each time it reviews). This time, a cheaper model wrote the design instead, and the strong model only ever reviewed it cold — never wrote a word of it. The question being tested: is that actually a *better* check, not just a cheaper one?

**The longer version:** Every prior design document in this project (including issue #64's own spec and plan) was authored by the strongest available reviewer model, then reviewed by fresh dispatches of that same model with no memory of its own authoring session. That's a real independence mechanism — but it's still, at some level, one model's judgment checking itself, even across a memory boundary. For issue #65, driven initially by cost (the strong model's usage was becoming expensive), a cheaper model authored the spec and implementation plan directly, with heavy live verification along the way (real database queries, a real API call to confirm a technical assumption, reading the installed library's actual source code rather than trusting documentation). The strong model's role was narrowed entirely to review — it never saw or influenced the authoring. This produced a genuinely different reviewer relationship to the document: a completely different mind reading it cold, not the same mind re-reading its own reasoning from a blank slate.

**Interview talking point:** "I care about the difference between 'a review that feels rigorous' and 'a review that's actually independent.' Having one model both author a design and review it — even across fresh, memory-wiped dispatches — is still one model's judgment checking itself. So for this fix, I split it: a cheaper model wrote the design, and the strong reviewer model never touched authoring at all, only reviewed cold. It was originally a cost decision, but it turned into a genuine experiment in review independence, and the results back it up — the same document then went through five review rounds and converged cleanly, which is the outcome you'd want from a real independence guarantee, not just a cheaper one."

---

### 29. What five rounds of independent review found — and the one root cause behind both real defects

**Simple version:** A design document went through five rounds of "get someone with zero memory of this document to read it critically." The first two rounds each found a real, serious problem the author had missed — and both problems turned out to be the exact same *kind* of miss, even though they were about completely different things.

**The longer version:** Round 1 found a warning about a real technical risk that had been placed as a code comment — but the actual action that would trigger that risk (changing a deployment setting) never involves opening that file, so the warning was invisible to the person who'd need it. Round 2, independently, found that new evaluation code called an external service directly without reusing an existing rate-limit safeguard already present elsewhere in the same file — live-reproduced as a real crash. Both misses turned out to fit one pattern: neither required deep technical misunderstanding (the author clearly understood both the deployment risk and the rate-limit mechanism in detail) — both required *tracing the actual operational path* once: who really encounters this, or does new code duplicate a pattern that already exists nearby. Neither the design-brainstorming skill's self-review checklist nor the implementation-planning skill's self-review checklist has a step for either of those things — both check a document's internal consistency, not its relationship to the real system or workflow around it. Rounds 3 through 5, run with the same adversarial instructions, found progressively less: one shippability-enforcement gap, one missing maintenance step, then nothing but a wording nitpick — a clear convergence curve, not random noise. The root-cause finding was written up as its own detailed, portable backlog item for a future session to design an actual process fix for, rather than being patched into the immediate document and forgotten.

**Interview talking point:** "The interesting result wasn't that review found bugs — it's that both real bugs, in completely different parts of the document, turned out to be the same *class* of miss: something you'd only catch by actually tracing the operational path once, not by re-reading the document more carefully. That told me the gap wasn't 'try harder,' it was a genuine hole in the self-review checklist itself — nothing in it asked 'who actually encounters this in real life' or 'does this duplicate something that already exists two files over.' I didn't just fix the two instances and move on — I wrote up the pattern as a concrete, evidence-backed process-improvement candidate for the review checklist itself, because the same gap will produce the same class of bug again on the next document if only the symptom gets fixed."
...
- **Retrieval never surfaces the actually-governing rule for a natural, real question (issue #64).** Symptom: asking about a team reduced below the minimum player count produced an answer built around a penalty-shoot-out-specific exception instead of the general rule. Investigation: called `searchChunks` directly against the live corpus (bypassing the HTTP layer and the password gate) and inspected the real top-8 candidates — the general rule's chunk (`Law 3 › 1`) was absent from all 8, while a superficially similar but scenario-specific chunk (`Law 10 › 3`, the shoot-out exception) was present and got cited. Root cause: an embedding-relevance gap — the general rule's phrasing doesn't score highly enough against this question's natural wording, even though it's the section that actually governs. Query decomposition doesn't help here either: `decompose()` returns the question unchanged, since it reads as one coherent scenario, not several distinct topics — this is a retrieval-depth problem, not a splitting problem. Disposition: filed as a tracked, non-blocking follow-up rather than hot-patched; not caused by and not fixable by the decomposition feature that was being tested.
- **Generation asserts an unsupported ruling when retrieved context is topically adjacent but scenario-mismatched (issue #65).** Symptom: the same handball-deflection question produced a correct honest hedge on one run and a confident, specific, incorrect ruling on an identical retry. Investigation: pulled the actual retrieved chunk text from the database — both runs retrieved the same passage, which governs a player scoring directly off their own hand/arm, not a defender's arm deflecting an opponent's shot. Root cause: the model correctly hedged when it recognized the mismatch, and incorrectly extrapolated a specific ruling from the wrong-scenario text when it didn't — a nondeterministic generation-grounding failure, not a retrieval failure (the retrieved text was topically relevant, just not about the asked scenario). Disposition: filed as a tracked follow-up; the honest first-try answer is evidence the app's "answer strictly from retrieved passages" design mostly works, but this shows a real edge where it can fail silently and convincingly.
...
- **Two harvested eval questions had wrong law-section citations that would have made the eval quietly lie.** Symptom: none visible from the outside — the eval would have kept reporting coverage numbers against citations that were simply wrong, one permanently understating true retrieval performance. Investigation: an independent AI review round queried the live corpus directly rather than trusting the citations' own explanation text, and found one citation pointed at the wrong subsection of a Law (the real rule lived one section over) while another question was missing a citation its own explanation should have included. Root cause: the citations were hand-curated by inference from general football knowledge, not verified character-by-character against the actual database rows before being written down. Fix: both corrected and the fix re-verified with a full eval re-run before merge — the mis-cited question reached full retrieval coverage immediately once its citation pointed at the right place. Prevention takeaway: an eval's own labels are only as trustworthy as their own verification; a plausible-sounding citation still needs checking against the real source, same discipline as Concept 23.
- **A stacked-branch git merge conflict, resolved by recognizing it as a known pattern rather than guessing.** Symptom: merging an old, long-open PR into a heavily-updated `main` hit a real conflict in the shared session journal file. Investigation: rather than resolving line-by-line, checked whether both sides were pure, non-overlapping *additions* to the same file region (each branch had appended a different dated entry at the same insertion point) — confirmed by diffing each side against their common ancestor and finding zero deletions in either direction. Resolution: a straightforward concatenation in chronological order — the exact same resolution class this project's own journal already documented as precedent from an earlier, structurally identical conflict. Prevention takeaway: recognizing "is this actually two additive changes competing for the same insertion point" turns a git conflict from a guessing exercise into a mechanical, verifiable resolution.
- **A risk mitigation written where the person who needed it would never see it (issue #65 design review).** Symptom: none visible in the document itself — it read as complete, and the author's own live verification had confirmed the underlying technical claim was correct. Investigation: an independent reviewer, reading the document cold, asked a simple question the author hadn't: *who actually performs the action this risk is about, and where would they be looking?* The risk was "changing a deployment configuration value could silently break answer generation," and the mitigation was a warning comment inside the source file that reads that configuration — but changing that configuration is a one-line edit in the hosting platform's dashboard, never touching that file at all. Root cause: the author verified the *technical* claim thoroughly (ran a real API call, read the actual library source) but never traced the *operational* path — simulating the actual human workflow that would trigger the risk, as opposed to verifying the risk's mechanics were understood correctly. Fix: moved the warning into the project's setup documentation (the artifact someone actually opens before changing that setting), and added a runtime check that logs a loud warning if the configuration is ever set to something unrecognized. Prevention takeaway: verifying a risk is real and verifying a mitigation is *reachable* are two different checks — thorough technical verification can coexist with a mitigation nobody will ever see.
- **New code silently skipped an existing safety mechanism sitting in the same file (issue #65 design review, round 2).** Symptom: none visible until independently, live-reproduced — a reviewer ran the new code's own logic three times in a row with no delay and hit a real rate-limit error from the external service it called. Investigation: the codebase already had a retry-with-backoff wrapper around calls to that same rate-limited service, built specifically because its free tier allows only 3 requests per minute — and the author had read that exact wrapper earlier in the same session while explaining an unrelated part of the plan. The new evaluation code, written later in the same session, called the service directly instead of reusing it. Root cause: not a knowledge gap — the author demonstrably understood the rate limit and the existing safeguard — but a failure to re-consult already-gathered context while writing new code that needed the identical protection. The plan's own verification step, if run as originally written, would have crashed roughly one-fifth of the way through every single time. Fix: extracted the shared retry logic into its own small file so both the old and new code import the same protection, rather than duplicating or reimplementing it. Prevention takeaway: reading an existing pattern once during a session doesn't guarantee it gets reused later in that same session — a self-review step that explicitly greps for "does a sibling function in this file already solve this exact problem" would have caught it without needing external review at all.
...
23. Statistical power matters in hand-rolled test bars: a 20% threshold checked at n=9 is really an 11% bar, not the number it claims to be.
24. Half-applying a good rule is still a gap: a design-revision-gets-its-own-branch rule correctly caught for a spec, missed for the paired implementation plan, until a retro closed it at the source.
25. Two RAG bugs found live, on the same day, that looked alike but weren't: a retrieval-layer miss (chunk never surfaced) vs. a generation-layer miss (chunk surfaced, wrong scenario, model guessed anyway).
26. Verification over trust: querying the actual data source — a database, a source doc, a memory file — instead of accepting a plausible-sounding claim, applied to debugging, eval-question curation, and correcting my own mistake alike.
27. Measure before you fix: reproduction (5 identical runs) plus a systemic check across unrelated questions, before committing to a fix's shape — evidence ruled out a general mechanism the intuitive "just in case" instinct would have built.
28. Citation integrity as a hard design constraint: a display-invisible `embedding_text` field feeds search, `content` stays the untouched official text shown to users — never edit what people see just to make it easier to find.
29. Fresh vs. resumed AI review, tested not assumed: a resumed reviewer did real work confirming fixes on two PRs, but a genuinely fresh reviewer still caught different real issues both times it was independently dispatched — real evidence for an open process-design question, not just intuition.
30. An eval's own labels need the same verification discipline as the code they measure — a hand-curated citation pointing at the wrong Law section makes a coverage number lie convincingly until someone checks the source.
31. A live production data-mutation gets a dry-run safety check plus an explicit, tooling-enforced human confirmation gate — evidence in hand doesn't replace the judgment call, it informs it.
32. Testing review independence, not assuming it: splitting design authorship from review (a different model writes, the strong reviewer only ever reads cold) as a deliberate experiment in whether that's a stronger independence guarantee than the same model reviewing its own prior work across fresh dispatches.
33. Two real review-round defects, in unrelated parts of a document, traced to one shared root cause (a missing "trace the actual operational path" step in self-review) — written up as a portable process-improvement candidate instead of just patched and forgotten.
...
- **Temperature** — a setting that controls how much randomness the model uses when choosing its next word; 0 always picks the single most-likely continuation, higher values allow less-likely (more "creative") ones through. This app sets it to 0 for answer generation, since a rules-lookup answer should never benefit from creative variance — issue #65's fix.
